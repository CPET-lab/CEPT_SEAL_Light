...
namespace seal
{
    ...
    void EncryptionParameters::save_members(ostream &stream) const
    {
        ...
        try
        {
            ...
            uint64_t plain_modulus_degree64 = static_cast<uint64_t>(plain_modulus_degree_);
            ...
            stream.write(reinterpret_cast<const char *>(&plain_modulus_degree64), sizeof(uint64_t));
    ...
    void EncryptionParameters::load_members(istream &stream, SEAL_MAYBE_UNUSED SEALVersion version)
    {
        ...
        try
        {
            ...
            // Read the plain_modulus_degree
            uint64_t plain_modulus_degree64 = 0;
            stream.read(reinterpret_cast<char *>(&plain_modulus_degree64), sizeof(uint64_t));

            // Only check for upper bound; lower bound is zero for scheme_type::none
            if (plain_modulus_degree64 > SEAL_POLY_MOD_DEGREE_MAX)
            {
                throw logic_error("plain_modulus_degree is invalid");
            }
            ...
            parms.set_plain_modulus_degree(safe_cast<size_t>(plain_modulus_degree64));
    ...
    void EncryptionParameters::compute_parms_id()
    {
        ...
        *param_data_ptr++ = static_cast<uint64_t>(poly_modulus_degree_);
        ...
        // Write the plain_modulus_degree. Note that it will always be positive.
        *param_data_ptr++ = static_cast<uint64_t>(plain_modulus_degree_);
