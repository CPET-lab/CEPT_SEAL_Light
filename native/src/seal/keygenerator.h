    ...
    */
    ...
    public:
        ...
        */
        ...
        /**
        // Added by Dice15
        Creates a KeyGenerator initialized with the specified SEALContext and a specified Hamming weight for the secret key.
        
        @param[in] context The SEALContext
        @param[in] hwt The Hamming weight of the secret key
        @throws std::invalid_argument if the encryption parameters are not valid
        */
        KeyGenerator(const SEALContext &context, size_t hwt);
    ...
    private:
        ...
        */
        ...
        // Added by Dice15
        /**
        Generates and returns the specified number of key switching keys from a given
        secret key to the secret key currently held by this KeyGenerator.

        @param[in] from_secret_key The source secret key to switch from
        @param[in] count The number of key switching keys to generate
        @param[in] save_seed If true, save seed instead of a polynomial
        @throws std::invalid_argument if count is zero or too large
        */
        KSwitchKeys create_kswitch_keys(const SecretKey &from_secret_key, std::size_t count, bool save_seed);
