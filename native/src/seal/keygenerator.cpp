...
namespace seal
{
    ...
    // Added by Dice15
    KeyGenerator::KeyGenerator(const SEALContext &context, size_t hwt) : context_(context), hwt_(hwt)
    {
        // Verify parameters
        ...
        if (hwt < 16 || hwt > (context_data.parms().poly_modulus_degree() / 2))
        {
            throw invalid_argument("invalid hwt");
        }
    ...
    void KeyGenerator::generate_sk(bool is_initialized)
    {
        ...
        if (!is_initialized)
        {
            ...
            {
                // Generate a ternary secret key with the specified Hamming weight
                sample_poly_ternary_hwt(parms.random_generator()->create(), parms, secret_key, hwt_);
            }
