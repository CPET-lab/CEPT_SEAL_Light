    ...
    */
    ...
    public:
        ...
        */
        ...
        {
            destination.resize(plain_slots_);
        ...
        */
        ...
        {
            if (destination.size() != plain_slots_)
            {
    ...
    private:
        // Modified by Dice15
        /**
        Provides encoding for sparsely packed ciphertexts for CKKS bootstrapping
        (see https://eprint.iacr.org/2018/153.pdf).

        For N = poly_modulus_degree and n = plain_modulus_degree,
        a plaintext polynomial m(Y) ¡ô Z[Y]/(Y^n + 1) maps to w = (w_j) with w_j = m(¥î^{3j}),  ¥î = exp(-2¥ði/n).
        The decoding map ¥ó gives z_j = m(¥î^{3j}) = w_j   (mod n/2).
        Hence z = (z_j)_{0 ¡Â j < N/2} = (w, w, ¡¦, w), i.e., w concatenated (N/n) times.
        The periodicity of ¥ó ensures that repeating w across slots yields the correct sparse encoding.
        */
        template <
            typename T, typename = std::enable_if_t<
                            std::is_same<std::remove_cv_t<T>, double>::value ||
            ...
            MemoryPoolHandle pool) const
        {
            ...
            if (values_size <= plain_slots_)
            {
                std::size_t repeat = slots_ / plain_slots_;
                for (std::size_t r = 0; r < repeat; r++)
                {
                    for (std::size_t i = 0; i < values_size; i++)
                    {
                        conj_values[matrix_reps_index_map_[(plain_slots_ * r) + i]] = values[i];
                        // TODO: if values are real, the following values should be set to zero, and multiply results
                        // by 2.
                        conj_values[matrix_reps_index_map_[(plain_slots_ * r) + i + slots_]] = std::conj(values[i]);
                    }
                }
            }
            else
        ...
        // Modified by Dice15
        /**
        Provides decoding for sparsely packed ciphertexts for CKKS bootstrapping
        (see https://eprint.iacr.org/2018/153.pdf).

        For N = poly_modulus_degree and n = plain_modulus_degree,
        since z = (w, w, ¡¦, w),  repeated (N/n) times,
        the first plain_slots_ entries of z correspond exactly to w.
        Thus decoding requires copying only plain_slots_ values from the inverse FFT result to the destination,
        while the remaining entries are redundant repetitions.
        */
        template <
            typename T, typename = std::enable_if_t<
                            std::is_same<std::remove_cv_t<T>, double>::value ||
        ...
        void decode_internal(const Plaintext &plain, T *destination, MemoryPoolHandle pool) const
        {
            ...
            for (std::size_t i = 0; i < plain_slots_; i++)
            {
        ...
        inline void encode_internal(
            ...
            MemoryPoolHandle pool) const
        {
            auto input = util::allocate<std::complex<double>>(plain_slots_, pool_, value);
            encode_internal(input.get(), plain_slots_, parms_id, scale, destination, std::move(pool));
        }
