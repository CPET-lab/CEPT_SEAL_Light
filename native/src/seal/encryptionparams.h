    ...
    */
    ...
    public:
        ...
        */
        inline void set_poly_modulus_degree(std::size_t poly_modulus_degree)
        {
            ...
            plain_modulus_degree_ = poly_modulus_degree;
        ...
        /**
        Sets the degree of the polynomial modulus parameter to the specified value 
        for sparsely packing of CKKS scheme. The plain_modulus_degree must be 
        a divisor of poly_modulus_degree and a power of two greater than or equal to 2.

        @param[in] plain_modulus_degree The new polynomial modulus degree of plaintext
        @throws std::logic_error if a valid scheme is not set and plain_modulus_degree
        is non-zero
        */
        inline void set_plain_modulus_degree(std::size_t plain_modulus_degree)
        {
            if (scheme_ != scheme_type::ckks && plain_modulus_degree)
            {
                throw std::logic_error("plain_modulus_degree is not supported for this scheme");
            }
            if (plain_modulus_degree & (plain_modulus_degree - 1))
            {
                throw std::invalid_argument("plain_modulus_degree must be a power of two greater than or equal to 2.");
            }
            if (poly_modulus_degree_ % plain_modulus_degree != 0)
            {
                throw std::invalid_argument("plain_modulus_degree must be a divisor of poly_modulus_degree.");
            }

            // Set the degree
            plain_modulus_degree_ = plain_modulus_degree;
        ...
        */
        ...
        // Added by Dice15
        /**
        Returns the degree of the plaintext polynomial modulus parameter.
        */
        SEAL_NODISCARD inline std::size_t plain_modulus_degree() const noexcept
        {
            return plain_modulus_degree_;
        }
            ...
            compr_mode_type compr_mode = Serialization::compr_mode_default) const
        {
            ...
            std::size_t members_size = Serialization::ComprSizeEstimate(
                util::add_safe(
                    ...
                    sizeof(std::uint64_t), // plain_modulus_degree_
    ...
    private:
        ...
        */
        ...
        std::size_t plain_modulus_degree_ = 0;
